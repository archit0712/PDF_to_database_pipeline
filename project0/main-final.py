...
def clean_incident_nature(nature_text):
    nature_text = nature_text.strip()  # Removes any leading/trailing spaces
    nature_text = re.sub(r'\s+', ' ', nature_text)  # Collapses multiple spaces into one
    nature_text = re.sub(r'\bEMSSTAT\b', '', nature_text, flags=re.IGNORECASE)  # Removes 'EMSSTAT' if present
    nature_text = re.sub(r'\b([A-Za-z\s]+?)(?:\s+[A-Z]+(?:\s+[A-Z]+)\b.|\|\s*\d+)$', r'\1', nature_text).strip()
    nature_text = re.sub(r'(?<!\d)(\d{7})(?!\d)', '', nature_text)  # Removes isolated ORI numbers (7 digits)
    nature_text = re.sub(r'\s*\d+$', '', nature_text)  # Removes trailing digits that might represent the ORI
    return nature_text.strip()
                ...
                for index, column_coordinate in enumerate(column_coordinates):
                    if index < len(column_coordinates) - 1:
                        if column_coordinate <= current_x < column_coordinates[index + 1]:
                            handle_multiline_data(column_values, index, current_line)
...
def handle_multiline_data(column_values, index, current_line):
